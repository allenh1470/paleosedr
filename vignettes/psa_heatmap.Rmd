...
excel_file_path <- "C:/Users/allen/OneDrive/Documents/Rstudio/Rstudio/paleosedr/extdata/2024.03.24_eklutna_compilation.xlsx"
...
```{r}
eklutna_heatmap_plot <- psa_heatmap(melted_df)
```

Excellent! You should now have a heatmap of the downcore particle size.
Like the previous plots, this is a ggplot and can be manipulated with
different color patterns and x/y limits. I personally like plotting
grain-size data with a logarithmic scale. If your data is dominated by
the clay to silt sizes, you can set it up like this:
...
```{r}
grid.arrange(grobs = list(eklutna_heatmap_plot, d50.plot, ), ncol = 7, align = "x")
```
